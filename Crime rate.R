...
# NA value detecting and filling (if needed)
df.num <- raw %>% select_if(is.numeric) %>% 
select(ViolentCrimesPerPop,everything())

df.char <- raw %>% select_if(is.character)

df.num %>% inspect_na()

df.char %>% inspect_na()

# df.num.mice <- df.num %>% mice(method='rf',seed=123)
# df.num <- df.num.mice %>% complete()

# rec.obj <- recipe(~., data=df.char) %>% 
# step_modeimpute(all_nominal()) %>% 
# prep(stringsAsFactors=FALSE)

# df.char <- bake(rec.obj, df.char)

# no need to run as there is no NA value in dataset.


# One Hote Encoding-------------
# df.char <- dummyVars(" ~ .", data = df.char) %>% 
...
g <- my_data %>% 
  ...
  labs(x="Predecited Power Level", 
       y="Observed Power Level",
